...
const { Function } = require('../parser/Function');
...
class Call {
  ...
  operate() {
    ...
    if (!(zapFunction instanceof Function)) {
      this.errorHandler.throwNewInvalidFunctionCalleeTypeError(this.callee.line, this.callee.col);
    };
...
class Evaluator {
  ...
  handleAssignment() {
    ...
    if (assigned.length != 1 || !expression.length) {
      this.errorHandler.throwInvalidValueAssignment(this.currentToken.line, this.currentToken.col);
    };
  ...
  handleBinary() {
    ...
    if (!leftNode.length || !rightNode.length) {
      this.errorHandler.throwInvalidBinaryExpression(this.currentToken.line, this.currentToken.col);
    };
  ...
  handleUnary() {
    // console.log('isUnary')
    let expr = this.rawExpression.slice(this.index);

    if (!expr.length) {
      this.errorHandler.throwInvalidUnaryExpression(
  ...
  handleCall() {
    ...
    this.next();
    if (!this.currentToken) {
      this.errorHandler.throwUnableToParseFunctionInvocation(
  ...
  handleOpenParen() {
    ...
    while (this.isInGroup()) {
      ...
      this.next();
      if (!this.currentToken) {
        this.errorHandler.throwExpectedRParens(
          this.previousToken.line, this.previousToken.col
        );
      };
    };
    let node = new Group(
  ...
  handleReserved() {
    this.errorHandler.throwUnexpectedKeyword(
      this.currentToken.line,
      this.currentToken.col,
    );
  };
  ...
  handleForbidden() {
    this.errorHandler.throwForbiddenSymbolInExpression(
      this.currentToken.line,
      this.currentToken.col,
    );
  };
    ...
    */
    ...
    while (this.currentToken && this.nextToken) {
      if (this.isLiteral(this.currentToken) && this.isLiteral(this.nextToken)) {
        // this.errorHandler.throwUnexpectedLiteral(
        //   this.nextToken.line,
        //   this.nextToken.col
        // );
      };
